...
const emptyJob = () => ({
  ...
  source:"app" // "app" = editable, "legacy" = read-only from Overview tab
});
...
// Parse "App Jobs" row into job object
function rowToJob(row) {
  ...
  return {
    ...
    pickup: row[11]||"", dropoff: row[12]||"",
    serviceType: row[13]||"Tow", price: row[14]||"", paymentType: row[15]||"Cash",
    status: status === "paid" ? STATUSES.PAID : ((!row[14] || row[14]==="") ? STATUSES.MISSING : STATUSES.UNPAID),
    notes: row[17]||"", vehiclePhoto: null, registrationPhoto: null,
    receiptGenerated: false, paidDate: status === "paid" ? (jobDate ? new Date(jobDate+"T12:00:00").toISOString() : null) : null,
    invoiceDetails: { subtotal:"", tax:"", total:"", tolls:"", ccFee:"" },
    source: "app"
  };
}

// Parse "United Towing Overview" legacy row into job object
// Fields: jobNumber, jobName (free text like "Gray Tesla"), date, payout, paymentType, receiptStatus, notes
function legacyToJob(row) {
  ...
  let jobDate = "";
  if (rawDate) {
    try { jobDate = new Date(rawDate).toISOString().split("T")[0]; } catch { jobDate = ""; }
  }
  ...
  // Parse the free-text job name into color/make/model as best we can
  ...
  // Determine status from receipt column and payment info
  ...
  // Build a customer name from notes or leave as job name context
  let customerName = "";
  // Check if any partner name appears in the job name or notes
  const combined = (jobName + " " + notes).toLowerCase();
  for (const p of PARTNERS) {
    if (combined.includes(p.toLowerCase())) { customerName = p; break; }
  }
...
async function fetchJobsFromSheets() {
  try {
    ...
    if (data.appJobs && Array.isArray(data.appJobs)) {
      result.appJobs = data.appJobs.map(rowToJob);
    }
    if (data.legacyJobs && Array.isArray(data.legacyJobs)) {
      result.legacyJobs = data.legacyJobs.map(legacyToJob);
    }
    // Backward compat: old format had data.jobs
    if (data.jobs && Array.isArray(data.jobs) && !data.appJobs) {
      result.appJobs = data.jobs.map(rowToJob);
    }
...
async function generateInvoicePDF(job) {
  ...
  ["CASH","CK#","CHARGE ACCOUNT"].forEach(function(pm){
    var cx2 = m+lw+8; cb(cx2, py2-3, pay.toUpperCase()===pm.split("#")[0].trim());
...
function CaptureForm({ onSubmit, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    const final={...job, isTest: testMode};
    ...
    setSyncing(false);
    setSubmitted(true);
    setTimeout(()=>{setSubmitted(false);setJob(emptyJob());setCustomCustomer(false);if(testMode){}},1800);
  };
  if (submitted) return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg}}>
      ...
      <div style={{fontSize:14,color:C.muted,marginTop:4}}>{testMode?"Saved to Test Jobs tab (not official)":"Saved to Google Sheets"}</div></div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:testMode?"#fff3e0":C.bg}}>
      <div style={{background:testMode?"#e65100":C.dark,color:"#fff",padding:"16px 20px",textAlign:"center"}}>
        ...
        <div style={{fontSize:12,color:"#ccc",marginTop:2}}>{testMode ? "\uD83E\uDDEA Test mode \u2014 won't save to official log" : "Quick job capture"}</div>
      </div>
      <div style={{padding:"16px 20px",maxWidth:420,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:8}}>{testMode?"Goes to Test Jobs tab only":"Syncs to Google Sheets automatically"}</p>
...
function StatusBadge({status,price,source}) {
...
function InvoicePanel({ job, onSave, onClose }) {
  ...
  const inputStyle={width:"100%",padding:"9px 10px",fontSize:14,borderRadius:6,border:`1.5px solid ${C.border}`,background:isLegacy?"#f5f5f5":C.white,boxSizing:"border-box",fontFamily:"inherit"};
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:C.white,borderRadius:12,width:"100%",maxWidth:600,maxHeight:"90vh",overflow:"auto",padding:24}}>
        ...
        {/* Legacy raw info banner */}
        {isLegacy && j.legacyRaw && (
          <div style={{background:C.lightBlue,borderRadius:8,padding:12,marginBottom:14,fontSize:13}}>
            <div><strong>Original entry:</strong> {j.legacyRaw.jobName}</div>
            <div><strong>Receipt status:</strong> {j.legacyRaw.receiptStatus || "\u2014"}</div>
            <div><strong>Payment type:</strong> {j.legacyRaw.paymentType || "\u2014"}</div>
          </div>
        )}

        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
          <div><label style={labelStyle}>Date</label>
          <input type={isLegacy?"text":"date"} value={j.jobDate} onChange={e=>!isLegacy&&update("jobDate",e.target.value)} readOnly={isLegacy} style={isLegacy?roStyle:inputStyle} /></div>
          <div><label style={labelStyle}>Time</label>
          <input type={isLegacy?"text":"time"} value={j.jobTime} onChange={e=>!isLegacy&&update("jobTime",e.target.value)} readOnly={isLegacy} style={isLegacy?roStyle:inputStyle} /></div>
        </div>
        ...
        <div style={{background:C.bg,borderRadius:8,padding:14,marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
            {[["Year","vehicle.year"],["Color","vehicle.color"],["Make","vehicle.make"],["Model","vehicle.model"]].map(([l,p])=>(
              <div key={p}><label style={labelStyle}>{l}</label>
              <input value={p.split(".").reduce((o,k)=>o[k],j)} onChange={e=>!isLegacy&&update(p,e.target.value)} readOnly={isLegacy} style={isLegacy?roStyle:inputStyle} /></div>
            ))}
          </div>
        ...
        <div style={{background:C.bg,borderRadius:8,padding:14,marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            ...
            <div><label style={labelStyle}>Phone</label><input value={j.customer.phone} onChange={e=>!isLegacy&&update("customer.phone",e.target.value)} readOnly={isLegacy} placeholder="347-722-0502" style={isLegacy?roStyle:inputStyle} /></div>
          </div>
        </div>

        {!isLegacy && <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
        ...
        <div style={{background:C.bg,borderRadius:8,padding:14,marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:isLegacy?"1fr 1fr":"1fr 1fr 1fr",gap:8,marginBottom:10}}>
            <div><label style={labelStyle}>Price</label><input value={j.price} onChange={e=>!isLegacy&&update("price",e.target.value)} readOnly={isLegacy} type={isLegacy?"text":"number"} style={isLegacy?roStyle:inputStyle} /></div>
            {!isLegacy && <div><label style={labelStyle}>Tolls</label><input value={j.invoiceDetails.tolls} onChange={e=>update("invoiceDetails.tolls",e.target.value)} type="number" placeholder="0" style={inputStyle} /></div>}
            <div><label style={labelStyle}>Payment type</label>
            {isLegacy ? <input value={j.paymentType} readOnly style={roStyle} /> :
            <select value={j.paymentType} onChange={e=>update("paymentType",e.target.value)} style={{...inputStyle,appearance:"auto"}}>{PAYMENT_TYPES.map(p=><option key={p} value={p}>{p}</option>)}</select>}
            </div>
          </div>
          {!isLegacy && <div style={{borderTop:`1px solid ${C.border}`,paddingTop:10,fontSize:14}}>
        ...
        {/* Status toggle - only for app jobs */}
        {!isLegacy && <div style={{marginBottom:14}}>
          <div style={{display:"flex",gap:0,borderRadius:8,overflow:"hidden",border:`1.5px solid ${C.border}`}}>
        ...
        {/* Status display for legacy */}
        {isLegacy && <div style={{padding:"12px 14px",borderRadius:8,background:j.status===STATUSES.PAID?C.lightGreen:(j.status===STATUSES.UNPAID?C.lightRed:C.lightYellow),textAlign:"center",fontSize:14,fontWeight:600,color:j.status===STATUSES.PAID?C.success:(j.status===STATUSES.UNPAID?C.danger:C.warning),marginBottom:14}}>
          {j.status===STATUSES.PAID?"Paid":(j.status===STATUSES.UNPAID?"Unpaid":"Missing info")}
        </div>}
        ...
        {!isLegacy && <button onClick={handleGeneratePDF} disabled={generating}
...
function Dashboard({ jobs, setJobs, onNewJob, onLogout, loading, onRefresh }) {
  ...
  const [sourceFilter, setSourceFilter] = useState("all"); // "all", "app", "legacy"
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      <div style={{background:C.white,borderBottom:`1px solid ${C.border}`,padding:"14px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div><div style={{fontSize:18,fontWeight:700,color:C.dark}}>United Towing</div><div style={{fontSize:12,color:C.muted}}>{loading ? "Loading..." : `${jobs.length} jobs (${legacyCount} legacy + ${appCount} app)`}</div></div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={onRefresh} style={{...baseBtn,background:C.bg,color:C.muted,fontSize:12,padding:"8px 12px"}} title="Refresh">{loading ? "\u23F3" : "\u21BB"}</button>
      ...
      <div style={{padding:"20px 24px",maxWidth:1200,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:16}}>
          {[["all","All jobs"],["legacy","Legacy ("+legacyCount+")"],["app","App ("+appCount+")"]].map(([k,label])=>(
        ...
        {/* Top unpaid - priority focus */}
        <div style={{background:C.white,borderRadius:10,padding:"16px 20px",boxShadow:C.cardShadow,marginBottom:16}}>
          <div style={{fontSize:15,fontWeight:700,color:C.danger,marginBottom:12}}>{"\u26A0\uFE0F"} Unpaid accounts \u2014 chase these</div>
          ...
          topAccounts.map(([name,data],i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:i<topAccounts.length-1?`1px solid ${C.border}`:"none"}}>
              <div><div style={{fontSize:14,fontWeight:600,color:C.dark}}>{name||"Unknown"}</div><div style={{fontSize:11,color:C.muted}}>{data.count} unpaid &middot; oldest {daysSince(data.oldest)}d ago</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:16,fontWeight:700,color:C.danger}}>{formatMoney(data.total)}</div></div>
            </div>
          ))}
          {topAccounts.length>0 && <div style={{marginTop:12,paddingTop:10,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",fontSize:14,fontWeight:700}}>
            <span>Total outstanding</span>
            <span style={{color:C.danger}}>{formatMoney(totalUnpaid)}</span>
          </div>}
        </div>
              ...
              <div key={j.id} onClick={()=>setEditing(j)} style={{padding:"12px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
                  ...
                  <div style={{textAlign:"right",marginLeft:12}}>
                    ...
                    <div style={{marginTop:4}}><StatusBadge status={j.status} price={j.price} source={j.source} /></div>
                  </div>
                </div>
                {(j.pickup||j.dropoff)&&<div style={{fontSize:12,color:C.muted}}>{[j.pickup,j.dropoff].filter(Boolean).join(" \u2192 ")}</div>}
                {j.source==="legacy"&&j.notes&&<div style={{fontSize:11,color:"#999",marginTop:2}}>{j.notes.slice(0,80)}{j.notes.length>80?"...":""}</div>}
              </div>
            ))}
          </div>
