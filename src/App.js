...
<button onClick={handleClear} style={{position:“absolute”,top:4,right:4,background:“rgba(0,0,0,0.6)”,color:”#fff”,border:“none”,borderRadius:“50%”,width:24,height:24,fontSize:14,cursor:“pointer”,display:“flex”,alignItems:“center”,justifyContent:“center”}}>×</button>
</div>
) : (
...
{!customCustomer ? (
<select value={job.customer.name} onChange={e=>{if(e.target.value===”**new**”){setCustomCustomer(true);update(“customer.name”,””);} else{update(“customer.name”,e.target.value);update(“customer.isPartner”,true);}}} style={{…inputStyle,appearance:“auto”}}>
...
<div style={{fontSize:13,color:C.muted,marginTop:2}}>{j.customer.name||“No customer”} · {formatDate(j.createdAt)}</div>
</div>
