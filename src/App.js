...
function StatusToggle({status,onChange}){return(<div style={{display:"flex",borderRadius:10,overflow:"hidden",border:`1.5px solid ${T.border}`}}>{[{v:ST.PAID,l:"Paid",c:T.accent},{v:ST.UNPAID,l:"Needs payment",c:T.red}].map(o=>(<div key={o.v} onClick={()=>onChange(o.v)} style={{flex:1,padding:"12px 0",textAlign:"center",fontSize:14,fontWeight:600,cursor:"pointer",background:status===o.v?o.c:T.surface,color:status===o.v?"#fff":T.muted,borderLeft:o.v===ST.UNPAID?`1px solid ${T.border}`:"none"}}>{o.l}</div>))}</div>)}
...
function Capture({onSubmit,onCancel}){
  ...
  const{sub,tax,cc:ccFee,total,svcSum}=totals(j.services,j.tolls,j.paymentType);
    ...
    <div style={{minHeight:"100vh",background:test?"#fff7ed":T.bg,fontFamily:T.font}}>
      ...
      <div style={{padding:"16px 20px",maxWidth:480,margin:"0 auto"}}>
        ...
        <Section title="Services & pricing"><SvcPricing services={j.services} onChange={s=>setJ(p=>({...p,services:s}))} />
          ...
          {total>0&&<div style={{borderTop:`1px solid ${T.border}`,marginTop:12,paddingTop:10,fontSize:14}}><div style={{display:"flex",justifyContent:"space-between",color:T.muted,marginBottom:3}}><span>Subtotal</span><span>{money(sub)}</span></div>{tax>0&&<div style={{display:"flex",justifyContent:"space-between",color:T.muted,marginBottom:3}}><span>Tax</span><span>{money(tax)}</span></div>}{ccFee>0&&<div style={{display:"flex",justifyContent:"space-between",color:T.muted,marginBottom:3}}><span>CC fee</span><span>{money(ccFee)}</span></div>}<div style={{display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:18,color:T.dark,marginTop:6}}><span>Total</span><span>{money(total)}</span></div></div>}</Section>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:12,fontFamily:T.font}}>
      <div style={{background:T.surface,borderRadius:14,width:"100%",maxWidth:560,maxHeight:"92vh",overflow:"auto",padding:24}}>
        ...
        <Section title="Services & pricing"><SvcPricing services={j.services} onChange={s=>setJ(p=>({...p,services:s}))} />
          ...
          {sub>0&&<div style={{borderTop:`1px solid ${T.border}`,marginTop:12,paddingTop:10,fontSize:14}}><div style={{display:"flex",justifyContent:"space-between",color:T.muted}}><span>Subtotal</span><span>{money(sub)}</span></div>{tax>0&&<div style={{display:"flex",justifyContent:"space-between",color:T.muted}}><span>Tax</span><span>{money(tax)}</span></div>}{cc>0&&<div style={{display:"flex",justifyContent:"space-between",color:T.muted}}><span>CC fee</span><span>{money(cc)}</span></div>}<div style={{display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:18,color:T.dark,marginTop:6}}><span>Total</span><span>{money(total)}</span></div></div>}</Section>
...
function Dashboard({jobs,setJobs,onNew,onOut,loading,refresh}){
  ...
  let list=pool;if(filt==="action")list=pool.filter(j=>j.status!==ST.PAID);else if(filt==="unpaid")list=unpaid;else if(filt==="missing")list=missing;else if(filt==="paid")list=paid;
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font}}>
      ...
      <div style={{padding:"16px 18px",maxWidth:960,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:18}}>{[{l:"Jobs",v:pool.length,bg:T.bg,c:T.dark},{l:"Collected",v:"$"+Math.round(colAmt).toLocaleString(),bg:"#ecfdf5",c:T.accent},{l:"Unpaid",v:"$"+Math.round(unpAmt).toLocaleString(),bg:"#fef2f2",c:T.red},{l:"Missing",v:missing.length,bg:"#fffbeb",c:T.amber}].map((m,i)=><div key={i} style={{background:m.bg,borderRadius:10,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:10,fontWeight:700,color:m.c,textTransform:"uppercase",letterSpacing:.5}}>{m.l}</div><div style={{fontSize:19,fontWeight:700,color:m.c,marginTop:2}}>{m.v}</div></div>)}</div>
        ...
        <div style={{background:T.surface,borderRadius:T.radius,padding:"14px 16px",boxShadow:T.shadow}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}><div style={{fontSize:15,fontWeight:700,color:T.dark}}>Jobs</div><div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}><input value={q} onChange={e=>{setQ(e.target.value);setShow(50)}} placeholder="Search..." style={{padding:"6px 10px",fontSize:12,borderRadius:8,border:`1.5px solid ${T.border}`,width:120,fontFamily:T.font,outline:"none"}} />{["action","unpaid","missing","paid","all"].map(f=><span key={f} onClick={()=>{setFilt(f);setShow(50)}} style={{padding:"5px 10px",borderRadius:14,fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",background:filt===f?T.dark:T.bg,color:filt===f?"#fff":T.muted}}>{f==="action"?"Needs action":f[0].toUpperCase()+f.slice(1)}</span>)}</div></div>
          {list.slice(0,show).map(j=>{const title=j.title||[j.vehicle.color,j.vehicle.make,j.vehicle.model].filter(Boolean).join(" ")||"No info";const sc=j.status===ST.PAID?T.accent:(j.status===ST.UNPAID?T.red:T.amber);const sl=j.status===ST.PAID?"Paid":(j.status===ST.UNPAID?"Unpaid":"Missing");return(<div key={j.id} onClick={()=>setEdit(j)} style={{padding:"11px 0",borderBottom:`1px solid ${T.bg}`,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:T.dark,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{title}{j.source==="migrated"&&<span style={{padding:"1px 5px",borderRadius:4,fontSize:9,fontWeight:700,background:"#eff6ff",color:"#2563eb",marginLeft:6,verticalAlign:"middle"}}>M</span>}{j.receiptMissing&&<span style={{padding:"1px 5px",borderRadius:4,fontSize:9,fontWeight:700,background:"#fffbeb",color:T.amber,marginLeft:4,verticalAlign:"middle"}}>No rcpt</span>}</div><div style={{fontSize:12,color:T.muted,marginTop:2}}>{j.customer.name||""}{j.customer.name&&" \u00B7 "}{fmtDate(j.jobDate)}</div></div><div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:14,fontWeight:700,color:(j.price&&!isNaN(j.price))?T.dark:T.amber}}>{(j.price&&!isNaN(j.price))?money(j.price):"No price"}</div><div style={{fontSize:11,fontWeight:600,color:sc,marginTop:2}}>{sl}</div></div></div>)})}
